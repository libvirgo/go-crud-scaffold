...
func (*UserActivity) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case useractivity.FieldID, useractivity.FieldType:
			values[i] = new(sql.NullInt64)
		case useractivity.FieldCreateTime, useractivity.FieldUpdateTime:
			values[i] = new(sql.NullTime)
		case useractivity.ForeignKeys[0]: // user_user_activity
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case useractivity.FieldType:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				ua.Type = int(value.Int64)
			}
		case useractivity.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field user_user_activity", value)
			} else if value.Valid {
				ua.user_user_activity = new(int)
				*ua.user_user_activity = int(value.Int64)
			}
		default:
